...
class MotionController:
    def __init__(self, info = None):
        ...
        self.vel_sub = Subscriber(conn_params=ConnParams.get("redis"), topic = info["base_topic"] + "/set", on_message = self.cmd_vel)
        self.logger.info("Created redis Subscriber {}".format(
            info["base_topic"] + "/set"
        ))

        self.motion_get_server = RPCService(conn_params=ConnParams.get("redis"), on_request=self.motion_get_callback, rpc_name=info["base_topic"] + "/get")
